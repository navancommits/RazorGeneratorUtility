...
namespace RazorGeneratorUtility
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            slnParentPath = Path.GetDirectoryName(args[1]);
            slnName= Path.GetFileName(args[1]);
            grandParentPath = args[2];
            option = Convert.ToInt16(args[0]);
        ...
        private static  void WriteRazorGenConfig(string concatLines)
        {
            if (!string.IsNullOrWhiteSpace(sitesConfigLocation))
            {
                if (!File.Exists(Path.GetDirectoryName(sitesConfigLocation) + @"\" + "RazorGen.Custom.Config"))
                {
                    using (FileStream fs = File.Create(Path.GetDirectoryName(sitesConfigLocation) + @"\" + "RazorGen.Custom.Config"))
                    {
                ...
                while (null != (currline = input.ReadLine()))
                {
                        ...
                        if (currline.Contains("Include=") && currline.Contains(".cshtml"))
                        {
                            if (!currline.Contains(buildAction))
                            {
                                ...
                                newline = "\t<" + buildAction + " Include=\"" + cshtmlPath + "\"/>\r";
                                concatLines += newline;
                                fileChanged = true;
                            }
                            else
        ...
        private static void AddRazorBuildReferenceinPackageConfig(string filePath)
        {
            ...
            using (var output = new StreamWriter("temp.csproj"))
            {
                ...
                while (null != (currline = input.ReadLine()))
                {
                    ...
                    {
                        currline =  "  <package id=\"RazorGenerator.MsBuild\" version=\"2.5.0\" targetFramework=\"net472\" developmentDependency=\"true\" />\r" + currline;
